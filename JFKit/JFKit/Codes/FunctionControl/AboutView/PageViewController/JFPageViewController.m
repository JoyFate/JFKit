...
- (void)initUI
{
    ...
    self.titleView = [[JFTitleView alloc] initWithFrame:titleFrame titles:self.titles images:self.images buttonMargin:self.titleMargin firstButtonMargin:self.firstMargin fontSize:self.titleFontSize];
    
    self.titleView.backgroundColor = RGB(248, 248, 248);
...
//    }];
    ...
    if (self.childControllerView.viewMoveBlock) {
        self.childControllerView.viewMoveBlock(0);
    }
}

- (void)refreshCurrView
{
    UIViewController *controller = [self.controllers objectAtIndex:self.currIndex];
    [controller viewDidAppear:YES];
}
