...
@property (nonatomic, copy) NSString *addButtonImage;                   // 添加按钮图片
...
- (instancetype)initWithAddImage:(NSString *)addImage deleteImage:(NSString *)deleteImage
{
    if (self = [super init]) {
        ...
        self.deleteButtonImage = deleteImage;
...
- (void)initUI
{
    ...
    for (NSInteger index = 0; index < photoMaxCount; index++) {
        ...
        ReviewPhotoTmpObject *tmpObject = [[ReviewPhotoTmpObject alloc] initWithAddImage:self.addButtonImage];
        
        // 删除按钮
        
        UIButton *deleteButton = [[UIButton alloc] init];
        [deleteButton setTag:index];
        [deleteButton setImage:[UIImage imageNamed:self.deleteButtonImage] forState:UIControlStateNormal];
        [deleteButton addTarget:self action:@selector(deleteButtonClick:) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:deleteButton];
        [deleteButton mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerX.equalTo(imageButton.mas_right);
            make.centerY.equalTo(imageButton.mas_top);
            make.width.height.mas_equalTo(fJFScreen(18*2 + 20*2));
        }];
        ...
        if (index == 0) {
            [imageButton setImage:[UIImage imageNamed:self.addButtonImage] forState:UIControlStateNormal];
            [imageButton setHidden:NO];
        }

        tmpObject.imageButton = imageButton;
        
        [imageButton setFrame:CGRectMake(x, fJFScreen(20 + 18), fJFScreen(140), fJFScreen(140))];
...
- (void)setDeleteButtonImage:(NSString *)deleteButtonImage
{
    _deleteButtonImage = [deleteButtonImage copy];
    
    for (ReviewPhotoTmpObject *obj in self.photosButtonArray) {
        [obj.deleteButton setImage:[UIImage imageNamed:deleteButtonImage] forState:UIControlStateNormal];
    }
}
...
{
    if (self.photosArray.count > 0) {
        NSMutableArray *tmpArray = [NSMutableArray arrayWithCapacity:self.photosArray.count];
        
        for (LGPhotoPickerBrowserPhoto *photo in self.photosArray) {
            [tmpArray addObjectSafe:photo.photoImage];
        }
...
@interface ReviewPhotoTmpObject ()

@property (nonatomic, copy) NSString *addPhotoImage;
...
- (void)setFirstAddButton
{
    ...
    [self.imageButton setImage:[UIImage imageNamed:self.addPhotoImage] forState:UIControlStateNormal];
}
