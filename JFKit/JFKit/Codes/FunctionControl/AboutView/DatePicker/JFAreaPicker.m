...
#define JFPickerButtonWidth  60
#define JFPickerButtonHeight 40

@interface JFAreaPicker () <UIPickerViewDataSource, UIPickerViewDelegate>
                     ...
                     complete:(AreaChooseBlock)complete
{
    if (self = [super initWithFrame:frame]) {
        self.provinceArray = (NSArray *)[areaDict objectForKey:JFPickerProvinceKey];
        self.cityDict      = (NSDictionary *)[areaDict objectForKey:JFPickerCityKey];
//        self.districtDict  = (NSDictionary *)[areaDict objectForKey:JFPickerDistrictKey];
        ...
        CGFloat buttonWidth = JFPickerButtonWidth;
        CGFloat buttonHeight = JFPickerButtonHeight;
...
- (void)sureButtonClick:(UIButton *)sender
{
    [UIView animateWithDuration:0.3f animations:^{
        ...
        [dict setObjectSafe:self.currentProvince forKey:JFPickerProvinceKey];
        [dict setObjectSafe:self.currentCity forKey:JFPickerCityKey];
        [dict setObjectSafe:self.currentCity forKey:JFPickerDistrictKey];
        
        if (self.completeBlock) {
            self.completeBlock([dict copy]);
        }
...
- (UIPickerView *)picker
{
    if (!_picker) {
        ...
        frame.origin.y = JFPickerButtonHeight;
        frame.size.height -= JFPickerButtonHeight;
